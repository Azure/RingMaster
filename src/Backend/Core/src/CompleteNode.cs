...
namespace Microsoft.Azure.Networking.Infrastructure.RingMaster.Backend
{
    ...
    public class CompleteNode : Node
    {
        ...
        public override Node TryGetChild(string name, bool allowWildcards = true)
        {
            ...
            // Detect the race condition -- when the key is added but value is not added yet, TryGetValue will return
            // true but value will be null. In this case, we simply retry.
            for (int i = 0; i < 16; i++)
            {
                var hasChild = this.childrenMapping.TryGetValue(name, out pd);
                if (!hasChild)
                {
                    break;
                }
        ...
        {
            // Do not demote complete nodes to plain nodes
